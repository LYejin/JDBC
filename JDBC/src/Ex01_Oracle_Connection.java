...
public class Ex01_Oracle_Connection {
	...
	public static void main(String[] args) throws Exception {
		...
		//Class.forName("oracle.jdbc.OracleDriver"); // new (생략 가능 자동로딩 ...)
		System.out.println("오라클 드라이버 로딩 ....");
		
		//loading 된 드라이버를 통해서 Oracle 서버 연결
		Connection conn= DriverManager.getConnection("jdbc:oracle:thin:@localhost:49161:xe","KOSA","1004");
		...
		//rs 객체를 통해서 DB 서버에 memory에 있는 정보를 조회
		//화면처리 (목록출력)
		while(rs.next()) { //현재 접근한 row가 있니?
			System.out.println(
					rs.getInt("empno") + " / " 
					+rs.getString("ename") +" / " 
					+rs.getInt("sal") + " / "
